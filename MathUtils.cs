...
public class MathUtils
{
	...
	public static float ProjectPointOnBezier(Vector3 point, float startInterval, float endInterval, int steps, Vector3 p0, Vector3 anchor0, Vector3 anchor1, Vector3 p1, out float distance)
	{
		...
		float step = (endInterval - startInterval) / (float)steps;
		float result = 0.0f;
		distance = float.MaxValue;

		for(int i = 0; i < steps; i++)
		{
			float t = startInterval + step * i;
			float distanceSquared = (GetPositionOnBezier(t, p0, anchor0, anchor1, p1) - point).sqrMagnitude;
	...
	{
		float stub = 0.0f;
		return ProjectPointOnBezier(point, startInterval, endInterval, steps, p0, anchor0, anchor1, p1, out stub);
	}

	public float ProjectPointOnBezierBinarySearch(Vector3 point, Vector3 p0, Vector3 anchor0, Vector3 anchor1, Vector3 p1)
	{
		float stub = 0.0f;
		return ProjectPointOnBezierBinarySearch(point, p0, anchor0, anchor1, p1, out stub);
	}

	public float ProjectPointOnBezierBinarySearch(Vector3 point, Vector3 p0, Vector3 anchor0, Vector3 anchor1, Vector3 p1, out float outDistance)
	{
		float t = ProjectPointOnBezier(point, 0.0f, 1.0f, 10, p0, anchor0, anchor1, p1);
		float delta = 1.0f / 10.0f;
		float distance = float.MaxValue;
		for(int i = 0; i < 4; i++)
		{
			t = ProjectPointOnBezier(point, t - delta, t + delta, 10, p0, anchor0, anchor1, p1, out distance);
			delta /= 9.0f;
		}
	...
	public static bool RotatedAABBContains(Bounds aabb, Vector3 point, Matrix4x4 transformMatrix)
	{
		Vector3 aabbSpacePoint = transformMatrix.inverse.MultiplyPoint(point);
		return aabb.Contains(aabbSpacePoint);
	}
}
